...
pub async fn run() -> anyhow::Result<()> {
    ...
    let login = env::var("login").unwrap_or("DarumaDockerDev".to_string());
    let owner = env::var("owner").unwrap_or("DarumaDockerDev".to_string());
    let repo = env::var("repo").unwrap_or("PrTest".to_string());
    ...
    listen_to_event(&login, &owner, &repo, events, |payload| {
        handler(&login, &owner, &repo, &trigger_phrase, payload)
    })
    .await;
