...
async fn handler() {
    ...
    router.insert("/postgres", vec![get(postgres)]).unwrap();
...
async fn postgres(_headers: Vec<(String, String)>, _qry: HashMap<String, Value>, _body: Vec<u8>) {
    let (client, connection) = tokio_postgres::connect("postgres://postgres.cvlrxycwyxkjtshjcuyq:Za0BHyJ595UPab94@aws-0-us-west-1.pooler.supabase.com:5432/postgres", NoTls).await.unwrap();
    ...
    send_response(200, vec![], row.into_bytes());
}
...
async fn test(_headers: Vec<(String, String)>, _qry: HashMap<String, Value>, _body: Vec<u8>) {
    ...
    let response =
        reqwest::get("https://github.com/juntao/test/blob/test1/src/github-pr-summary.rs")
            .await
            .unwrap();
...
async fn query(
    ...
    let resp = match city {
        Some(c) => get_weather(c).await.map(|w| {
            format!(
                "Today: {},
Low temperature: {} °C,
High temperature: {} °C,
...
async fn get_weather(city: &str) -> Result<ApiResult, String> {
    let api_key = "09a55b004ce2f065b903015e3284de35";
    let query_str = format!(
        "https://api.openweathermap.org/data/2.5/weather?q={city}&units=metric&appid={api_key}"
    );

    match reqwest::get(query_str).await {
        Ok(res) => serde_json::from_slice::<ApiResult>(res.bytes().await.unwrap().as_ref())
            .map_err(|_| {
